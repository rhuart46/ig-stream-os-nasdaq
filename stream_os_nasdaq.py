...
def creer_ordre_stop(epic, ENTREE, headers):
    ...
    if order_response.status_code == 200:
        print(f"Ordre placé sur {epic} avec succès au niveau {ENTREE} pour un stop loss de {SL} et un take profit de {TP}")
...
def validation_ordre(ticker, headers):
    ...
    for ENTREE in NIVEAUX_ENTREE:
        ...
        if (seuil_reactivation[ENTREE] and not ordres_ouverts[ENTREE] and not ordres_crees[ENTREE] and signal_volume==True and signal_momentum==True):
            if (deal_ids[ENTREE] is None or deal_ids[ENTREE] == '') and ENTREE - ACTIVATION <= ticker.offer <= ENTREE + ACTIVATION:
                ...
                if deal_id:
                    if attendre_confirmation_ordre(deal_id, headers):
                        ...
                        logging.info(f"ordres_crees[{ENTREE}] = {ordres_crees[ENTREE]} avec deal_ids[{ENTREE}] = {deal_ids[ENTREE]}")
                    else:
